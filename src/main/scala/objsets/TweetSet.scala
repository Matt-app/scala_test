...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def mostRetweeted: Tweet =
    var mrt = Tweet("", "", -1)
    this.foreach((x: Tweet) => if mrt.retweets < x.retweets then mrt=x)
    mrt

  def descendingByRetweet: TweetList =
    Cons(this.mostRetweeted, this.remove(this.mostRetweeted).descendingByRetweet)
//    amazing!!!!!!!! what have i done!! beautiful
...
//  val tmpB = GoogleVsApple.appleTweets
//  var set1 = Empty()
...
//  var set5 = set4c.incl(d)
//  val f1 = set4c.filter(tw => tw.retweets == 20)
//  f1.foreach((x: Tweet) => println(x.text))
//  val u1 = set4d.union(set4c)
//  u1.foreach((x: Tweet) => println(x.user))
//  println(u1.mostRetweeted)
